...
> 🔴 **9/22 정정 — 폴백률은 `mode:` 로 세라.** 헤더의 `ai:` 는 **라우트가 타임아웃 경주에 걸렸는가**만 말한다. 엔진이 안에서 폴백하면 `ai:gemini` 인 채로 규칙 코스가 나간다 → 그래서 재측정을 「폴백 0/8」로 잘못 읽었고, 본문 `generationMode` 로 다시 세면 **1/8** 이다(**5/8 → 1/8** 이 맞다). 헤더에 `mode:` 를 실어 막았다(`abce82c`, 570 tests GREEN, 운영 확인 `mode:ai`). 같은 날 **실제 화면 종단도 처음 확인**: 위저드 → 로딩(2→7→14→24초째, 진행바 3→12→23→40%, 멘트 순환) → 결과(지도·공유·바꾸기·검증 요약) 정상, 총 40초. 내부 폴백 **사유**는 아직 서버 로그에만 있다 — 다음 후보는 사유도 헤더에 싣기.
