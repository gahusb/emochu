...
5. `attribution` 이 이번 실행에서 처음 ✅ 로 바뀌었으면 `Needs Human Review` 에 `file:line` 과 함께 "사람이 렌더링 확인 필요"를 남긴다
...
**`attribution` ✅ 의 의미**: 주석이 아닌 소스에 매칭 문자열이 존재한다는 뜻일 뿐, 화면에 렌더링된다는 증거는 아니다.
...
- `api-list` 누락/초과 있음 → 활용 API 목록이 바뀐 것. 제출 서류와 어긋나므로 **즉시 사람 호출**
- `serviceUrl` 이 **응답은 받았지만 200이 아님** → 배포 문제. **즉시 사람 호출**
- `serviceUrl` 이 **타임아웃(10초)/네트워크 오류로 무응답** → 미확인(배포 중단 vs 일시 장애). 사람이 재확인, 즉시 비상 호출은 아님
