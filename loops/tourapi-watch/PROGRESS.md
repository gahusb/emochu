...
> 최신 상태는 이 상단을 기준으로 읽는다. 아래 과거 기록의 일일 호출 누적 권고는 현재 실행 지침이 아니다.
...
- Output: `outputs/api-health-2026-09-09.md`
- 별도 비인증 호스트 진단에서 원인 코드 **EACCES** 확인. 권한을 확보한 공개 호스트 연결은 HTTP 400 응답까지 도달했다. 원본 보고서의 “서비스가 곧 깨진다” 문구는 이 관측만으로 뒷받침되지 않는다.
- 같은 날짜 스모크는 재실행하지 않았다. 원본 보고서 보존.
- 사용자 요청 개발의 별도 코스 품질 실측에서는 KorService2 4개 + KorWithService2 1개 오퍼레이션이 HTTP 200 / 0000. 이 결과는 전체 11개 생존 확인을 대신하지 않는다.
- 실측 상세: `docs/2026-09-09-실데이터-검증-개발결과.md`.

## Open Items

- `areaCode2`·`categoryCode2`를 포함한 전체 11개 상태는 이번 스모크에서 **미확인**.
- 이전 9/1 정상 기록 이후 totalCount 비교는 이번 네트워크 실패로 불가능하다.
- 무장애·기상청·AI의 코스 품질 검증과 이 스모크의 건강 상태 검사를 혼동하지 않는다.

## Blockers

- 제한된 실행 환경의 외부 네트워크 접근 권한. API 자체 장애로 단정하지 않는다.

## Needs Human Review

- 폐기 감시 대상 2개 오퍼레이션의 실제 상태는 다음 허용된 수동 점검에서 확인해야 한다.
- 현재 smoke 스크립트는 전송 실패를 FAIL로 표기하고 감시 대상도 재시도한다. 환경 실패·폐기 신호를 분리하는 개선이 필요하다(이번 루프 실행에서는 소스 수정하지 않음).

## Next Run Should

1. 같은 날짜 기존 보고서 여부를 먼저 확인한다. 자동 스케줄은 만들지 않는다.
2. 다음 수동 실행은 네트워크 권한을 확인한 환경에서 `node loops/tourapi-watch/smoke.mjs`.
3. HTTP 이전 오류는 API 폐기나 인증 실패로 해석하지 않는다. 확인된 응답만으로 판정한다.
4. 호출 이력 자체가 아니라 실제 기능의 데이터 활용·품질을 증빙한다.
