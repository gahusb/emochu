...
| 신호 | 판정 | gate.mjs exit |
|---|---|---|
| 3종 전부 exit 0 + 테스트 ≥ 61 | ✅ GREEN | **0** |
| 3종 전부 exit 0 + **테스트 < 61** (또는 판독 실패) | 🟡 WARN — 회귀 의심 | **1** |
| 하나라도 exit ≠ 0 | 🔴 RED | **1** |

> 🔴 **테스트 개수가 줄었는데 전부 통과했다고 GREEN으로 넘기지 마라.** 테스트가 삭제됐거나 skip 됐을 수 있다.
> 🔴 **WARN도 exit 1이다.** GREEN만 exit 0 — gate.mjs 자체가 이 셋을 구분해서 종료 코드를 낸다. exit 코드만 보고 GREEN이라 단정하지 말고, 리포트 헤딩(GREEN/WARN/RED)을 반드시 확인한다.
